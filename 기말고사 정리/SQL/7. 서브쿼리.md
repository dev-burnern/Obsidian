...
**예시 상황:**
```
-- 1단계: 오상우 사원의 급여 조회 SELECT salary FROM y_emp WHERE emp_name = '오상우'; -- 결과: 630 -- 2단계: 630보다 급여가 많은 사원 조회 SELECT salary FROM y_emp WHERE salary > 630; -- 서브쿼리로 한 번에 처리 SELECT salary FROM y_emp  WHERE salary > (SELECT salary FROM y_emp WHERE emp_name = '오상우');```
